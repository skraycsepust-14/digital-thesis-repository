...
const AdminDashboardPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const [deptRes, statusRes, pendingRes, supervisorRes] =
          await Promise.all([
            axios.get(
              "http://localhost:5000/api/theses/analytics/by-department",
              config
            ),
            axios.get(
              "http://localhost:5000/api/theses/analytics/by-status",
              config
            ),
  ...
  if (loading)
    return (
      <div className="admin-dashboard text-center">Loading dashboard...</div>
    );
  if (error)
    return (
      <div className="admin-dashboard alert alert-danger text-center">
        {error}
      </div>
    );
  ...
  return (
    <div className="container admin-dashboard">
      ...
      <div className="tabs d-flex justify-content-center mb-4">
        <button
          className={`btn ${
            activeTab === "analytics" ? "btn-primary" : "btn-outline-primary"
          } mx-2`}
      ...
      {activeTab === "list" && (
        <>
          <div className="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            ...
            <div>
              <Link to="/upload-thesis" className="btn btn-primary me-2">
                Upload New Thesis
              </Link>
              <CSVLink
                data={pendingTheses}
                headers={csvHeaders}
                filename="pending-theses.csv"
                    ...
                    <div className="card-body">
                      ...
                      <div className="d-flex action-buttons">
                        <button
                          ...
                          onClick={async () => {
                            await axios.put(
                              `http://localhost:5000/api/theses/approve/${thesis._id}`,
                              {},
                              {
                                headers: { "x-auth-token": token },
                              }
                            );
                            setPendingTheses((prev) =>
                              prev.filter((t) => t._id !== thesis._id)
                            );
                        ...
                        <button
                          ...
                          onClick={async () => {
                            await axios.put(
                              `http://localhost:5000/api/theses/reject/${thesis._id}`,
                              {},
                              {
                                headers: { "x-auth-token": token },
                              }
